...
namespace Macomber_Map.User_Interface_Components
{
    ...
    public partial class MM_Data_Entry : MM_Form
    {
        ...
        public MM_Data_Entry()
        {
            ...
            srMain.ItemSelectionChanged += new Search_Results.ItemSelectionDelegate(srMain_ItemSelectionChanged);
        ...
        private static DataTable LoadOptionsTable(Type TargetType)
        {
            ...
            else if (TargetType == typeof(MM_Generation_Type))
                Options = MM_Repository.GenerationTypes.Values;
            else if (TargetType == typeof(MM_LogicalUnit))
                Options = MM_Repository.Units.Values.OfType<MM_LogicalUnit>();
        ...
        private void SubmitButton_Click(object sender, EventArgs e)
        {
            if (SelectedElement == null)
                Program.MessageBox("Please select an element to be violated.", Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Information);
            else
            try
            {                            
        ...
        {
            if (InvokeRequired)
                BeginInvoke(new Search_Results.ItemSelectionDelegate(SelectElement), SelectedElement);
            else
            {
        ...
        {
            SelectedElement.TriggerValueChange();
        }
    }
}