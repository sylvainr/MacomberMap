...
namespace Macomber_Map.User_Interface_Components
{
    ...
    /// </summary>
    public partial class Mini_Map : UserControl
    {
        ...
        public Violation_Viewer Violations;
        ...
        /// </summary>
        public Mini_Map(bool IsNetworkMap)
        {
            InitializeComponent();
        ...
        public void SetControls(Network_Map networkMap, Violation_Viewer Viol, DataSet_Base BaseData, MM_Line[] BorderingLines, PointF TopLeft, PointF BottomRight)
        {
            ...
            foreach (MM_Line ThisLine in BorderingLines)
                if (!this.BorderingLines.ContainsKey(ThisLine))
                    if (BaseData.Data.Tables["Substation"].Rows.Find(ThisLine.ConnectedStations[0]) != null)
        ...
        /// <param name="networkMap">The associated network map</param>
        /// <param name="Viol">The violations viewer, showing currently-displayed violations</param>
        public void SetControls(Network_Map networkMap, Violation_Viewer Viol)
        {
        ...
        private void HandleResize(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (PointF LatLng in StateBoundary.Coordinates)
                    NewPoint.Add(ConvertPoint(LatLng));
        ...
        private PointF ConvertPoint(PointF LatLng)
        {
            if (!ShowRegionalView)
                ...
                return new PointF(
                    (float)DisplayRectangle.Width * (LatLng.X - TopLeft.X) / (BottomRight.X - TopLeft.X),
                    (float)DisplayRectangle.Height * (LatLng.Y - BottomRight.Y) / (TopLeft.Y - BottomRight.Y));
        }
        ...
        private void HandleRepaint(object sender, PaintEventArgs e)
        {
            ...
            {
                Program.LogError("Error in mini-map repaint: " + ex.Message);
            }
        }
        ...
        private void DrawSubstation(Graphics g, MM_Substation Substation, bool DisplayName)
        {
            ...
            PointF SubstationPoint = ConvertPoint(Substation.LatLong);
        ...
        {
            PointF StartPoint = ConvertPoint(Line.ConnectedStations[0].LatLong);
            PointF EndPoint = ConvertPoint(Line.ConnectedStations[1].LatLong);
        ...
        private void DrawRequestedIndicators(Graphics g)
        {
            ...
            float FlowIn = 0, FlowOut = 0, Gen = 0f, Ld = 0f, Caps = 0f, Reacs = 0f, CapN = 0, ReacN = 0, CapA = 0, ReacA = 0, CapAs = 0, ReacAs = 0, GenC = 0, GenEC = 0;
            if (BorderingLines == null)
            {
            ...
            else
            {
                ...
                if (BaseData.Data.Tables.Contains("Unit"))
                    ...
                    {
                        float Val = ShowMVA ? (dr["TEID"] as MM_Unit).Estimated_MVA : (dr["TEID"] as MM_Unit).Estimated_MW;
                        float UnitMW = (dr["TEID"] as MM_Unit).Estimated_MW;
                        float UnitHASL = (dr["TEID"] as MM_Unit).HASL;
                        float UnitHSL = (dr["TEID"] as MM_Unit).HSL;
                        if (!float.IsNaN(Val))
                            Gen += Val;
                        if (!float.IsNaN(UnitMW) && !float.IsNaN(UnitHASL) && UnitHASL > UnitMW)
                            GenC += UnitHASL - UnitMW;
                        if (!float.IsNaN(UnitMW) && !float.IsNaN(UnitHSL) && UnitHSL > UnitMW)
                ...
                if (BaseData.Data.Tables.Contains("Capacitor"))
                    foreach (DataRow dr in BaseData.Data.Tables["Capacitor"].Rows)
                    {
                        ...
                        if (Cap.Open)
                        {
                            Caps += Cap.Nominal_MVAR;
                            CapN++;
                        }
                        else if (!float.IsNaN(Cap.Estimated_MVAR))
                        {
                            CapAs += Cap.Estimated_MVAR;
                            CapA++;
                        }
            ...
            if (ShowLoad && Ld > 0)
            {
                String LoadToDraw = "Load:\n" + Ld.ToString("#,##0") + (ShowMVA ? "\nmva" : "\nmw");
                if (Island != null)
                    LoadToDraw = "Load:\n" + Island.Island_Load.ToString("#,##0") + "\nmw";
            ...
            if (ShowCaps && Caps != 0)
            {
                String CapsToDraw = String.Format("Caps:" + (CapN > 0 ? "\nAvail:{1:#,##0}/{2:#,##0}" : "") + (CapA > 0 ? "\nUsed:{3:#,##0}/{4:#,##0}" : ""), ShowMVA ? "mva" : "mvar", CapN, Caps, CapA, CapAs);
                //" + CapN.ToString("#,##0") + "/\n" + Caps.ToString("#,##0") + "\nAvail.";
            ...
            if (ShowReactors && Reacs != 0)
            {
                String ReacsToDraw = String.Format("Reacs:" + (ReacN > 0 ? "\nAvail:{1:#,##0}/{2:#,##0}" : "") + (ReacA > 0 ? "\nUsed:{3:#,##0}/{4:#,##0}" : ""), ShowMVA ? "mva" : "mvar", ReacN, -Reacs, ReacA, -ReacAs);
        ...
        private void DrawViolation(Graphics g, MM_AlarmViolation ViolToDraw, bool Selected, bool SingleViolation)
        {
            ...
            {
                if (float.IsNaN((ViolToDraw.SubstationOrLine as MM_Substation).LatLong.X))
                    return;

                PointF StationPoint = ConvertPoint((ViolToDraw.SubstationOrLine as MM_Substation).LatLong);
            ...
            {
                if (float.IsNaN((ViolToDraw.SubstationOrLine as MM_Line).ConnectedStations[0].LatLong.X))
                    return;

                PointF FirstPoint = ConvertPoint((ViolToDraw.SubstationOrLine as MM_Line).ConnectedStations[0].LatLong);
                PointF SecondPoint = ConvertPoint((ViolToDraw.SubstationOrLine as MM_Line).ConnectedStations[1].LatLong);
        ...
        public void ZoomPanToPoint(PointF TargetLatLng)
        {
            //Simply set our new lat/long
            Coord.Center = TargetLatLng;
            ...
            PointF TargetPos = ConvertPoint(TargetLatLng);
            ...
            ZoomPanPoints.Enqueue(new KeyValuePair<PointF, float>(TargetLatLng, Coord.ZoomLevel));
            ZoomPanPoints.Enqueue(new KeyValuePair<PointF,float>(TargetLatLng, 24f));

            * //These following lines were double commented.
            * //Now zoom back in
            * float ZoomInLevel = 24f;
            * for (float Zoom = PanLevel; Zoom < ZoomInLevel; Zoom++)
            *    ZoomPanPoints.Enqueue(new KeyValuePair<PointF, float>(TargetLatLng, Zoom));
