...
namespace Macomber_Map.Data_Elements
{
    ...
    public class MM_Boundary : MM_Element
    {
        ...
        public float LMPMaximum
        {
            get
            {
                ...
                foreach (MM_Substation Sub in this.Substations)
                    if (Sub.Units != null)
                        foreach (MM_Unit Unit in Sub.Units)
                            if (Unit.Permitted && float.IsNaN(Unit.LMP) || !float.IsNaN(Unit.LMP))
        ...
        {
            byte[] inBytes;
            if (sRd is System.Data.OracleClient.OracleDataReader)
                inBytes = (byte[])(sRd as System.Data.OracleClient.OracleDataReader).GetOracleLob(sRd.GetOrdinal("COORDINATES")).Value;
            else
                inBytes = (byte[])sRd["COORDINATES"];
            Coordinates = new PointF[inBytes.Length / 8];
            ...
            : base(xE, AddIfNew)
        {
            ...
            {
                String[] Coords = xE["Coordinates"].InnerText.Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
                ...
                Coordinates = OutCoordinates.ToArray();
        ...
        public XmlElement BoundaryToXML(XmlDocument xDoc)
        {
            ...
            foreach (PointF LatLng in Coordinates)
                outElem.InnerText += LatLng.X.ToString() + "," + LatLng.Y.ToString() + ",";
            return outElem;
        }
        #endregion
        ...
        {
            if ((Coord.BottomRight.X < Min.X) || (Coord.TopLeft.Y < Min.Y))
                return false;
            else if ((Coord.TopLeft.X > Max.X) || (Coord.BottomRight.Y > Max.Y))
                return false;
            return true;
        }
