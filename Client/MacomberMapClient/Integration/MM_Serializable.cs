...
using System.Xml;
using MacomberMapClient.Integration.ReflectionOrm;
...
namespace MacomberMapClient.Integration
{
    ...
    /// This class holds information on a serializable library
    /// </summary>
    public class MM_Serializable
    {
        ...
        #region Value retrieval/assignment
        /// <summary>
            ...
            if (ColValue is DBNull == false)
                try
                {
                    if (ColName.Equals("Winding1", StringComparison.CurrentCultureIgnoreCase))
                        ...
                        MM_System_Interfaces.WriteConsoleLine("Unable to locate attribute " + ColName + "!");
                }
                catch (Exception ex)
                {
        ...
        {
            if (_propMap == null)
                _propMap = this.GetType().GetPropertyMap();
            ...
            foreach (XmlAttribute xAttr in XmlToRead.Attributes)
            {
                ...
                {
                    FastProperty pi;
                    if (_propMap.TryGetValue(xAttr.Name, out pi))
                        ConvertAndStoreValue(pi, pi.ReferenceType, xAttr.Value, AddIfNew);
                ...
                catch (Exception ex)
                {
                    ...
                    MM_System_Interfaces.LogError("Problem Loading xml attribute: " + xAttr.ToString());
                }
            }
        }
        ...
        private static void AssignValue(FastProperty fpI, Object OutgoingValue, Object Target)
        {
            fpI.Set(Target, OutgoingValue);
        }
        ...
        public static object RetrieveConvertedValue(Type OutgoingType, Object OutgoingValue, object Target, bool AddIfNew)
        {
            try
            {
                ...
                else if (OutgoingType == typeof(List<MM_EMS_Command>))
                {
                ...
                {
                    string[] vals = OutgoingValue.ToString().Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
                    if (vals == null || vals.Length == 0)
                        return null;
                    int[] rt = new int[vals.Length];
                    for (int i = 0;i < vals.Length; i++)
                    {
                        rt[i] = Int32.Parse(vals[i]);
                    }
                    return rt;
                }
                ...
                else if (OutgoingType == typeof(MM_Unit_Type) && OutgoingValue is String)
                {
                ...
                else if (OutgoingType == typeof(List<MM_Contingency>) && OutgoingValue is string)
                {
                    String[] splStr = ((String)OutgoingValue).Split(',');
                    List<MM_Contingency> OutVal = new List<MM_Contingency>(splStr.Length);
                    for (int a = 0; a < OutVal.Count; a++)
                    {
                        MM_Contingency con = null;
                        MM_Repository.Contingencies.TryGetValue(splStr[a], out con );
                        if (con != null)
                            OutVal[a] = con;
                        MM_Element el = null;
                        if (OutVal[a] == null && splStr[a].Length > 1 && char.IsDigit(splStr[a][0]))
                            MM_Repository.TEIDs.TryGetValue(Int32.Parse(splStr[a]),  out el);
                        
                        if (el != null)
                            OutVal[a] = (MM_Contingency)el;
                    }
                    return OutVal;
                }
                else if (OutgoingType == typeof(MM_Contingency[]) && OutgoingValue is string)
                {
                    String[] splStr = ((String)OutgoingValue).Split(',');
                    MM_Contingency[] OutVal = new MM_Contingency[splStr.Length];
                    for (int a = 0; a < OutVal.Length; a++)
                        MM_Repository.Contingencies.TryGetValue(splStr[a], out OutVal[a]);
                    return OutVal;
                }
                else if (OutgoingValue == null || string.IsNullOrWhiteSpace(OutgoingValue.ToString()))
                    return null;
                else if (OutgoingType == typeof(MM_Chart_Data[]))
                    return null;
                else
        ...
        /// <param name="AddIfNew">Whether to add a new element to our master repository</param>        
        private void ConvertAndStoreValue(FastProperty fpI, Type OutgoingType, String OutgoingValue, bool AddIfNew)
        {
            ...
            else if (typeof(MM_Element).IsAssignableFrom(OutgoingType)) // more concise
                AssignValue(fpI, Data_Integration.LocateElement(Int32.Parse(OutgoingValue), OutgoingType, AddIfNew), this);
            else if (OutgoingType == typeof(MM_Contingency[]) || OutgoingType == typeof(List<MM_Contingency>))
            {
                ...
                foreach (String str in OutgoingValue.Split(','))
                    if (MM_Repository.Contingencies.TryGetValue(str, out FoundCtg))
                        OutCtg.Add(FoundCtg);
                if (OutgoingType == typeof(List<MM_Contingency>))
                    AssignValue(fpI, OutCtg, this);
                else
            ...
            else if (OutgoingType == typeof(Rectangle))
            {
                ...
                AssignValue(fpI, MM_Repository.FindKVLevel((string)OutgoingValue), this);
            else if (OutgoingType == typeof(MM_Unit_Type))
            ...
            else if (OutgoingType == typeof(Single[]))
            {
                ...
                var OutPts = Array.ConvertAll(OutgoingValue.Split(','), float.Parse);
                AssignValue(fpI, OutPts, this);
            }
            else if (OutgoingType == typeof(String[]))
            {
                String[] splStr = OutgoingValue.Split(new char[]{','}, StringSplitOptions.RemoveEmptyEntries);
                if (splStr != null && OutgoingValue != null)
                    AssignValue(fpI, splStr, this);
            }
            ...
            else if (OutgoingType == typeof (MM_Boundary[]) || OutgoingType == typeof (List<MM_Boundary>))
            {
                List<MM_Boundary> OutBounds = new List<MM_Boundary>();
                foreach (String str in ((string) OutgoingValue).Split(','))
                    if (!String.IsNullOrEmpty(str))
                    {
                        if (MM_Repository.Counties.ContainsKey(str))
                            OutBounds.Add(MM_Repository.Counties[str]);
                        else if (char.IsDigit(str[0]) && MM_Repository.TEIDs.ContainsKey(Int32.Parse(str)))
                        {
                            MM_Element b = null;
                            MM_Repository.TEIDs.TryGetValue(Int32.Parse(str), out b);
                            OutBounds.Add(b as MM_Boundary);
                        }
                    }
                if (OutgoingType == typeof (MM_Boundary[]))
                    AssignValue(fpI, OutBounds.ToArray(), this);
                else
                    AssignValue(fpI, OutBounds, this);
            }
            ...
            else if (OutgoingType == typeof(List<MM_Transformer>))
            {
                String[] splStr = OutgoingValue.Split(',');
                List<MM_Transformer> OutTransformers = new List<MM_Transformer>(splStr.Length);
                for (int a = 0;a < splStr.Length;a++)
                    OutTransformers.Add(Data_Integration.LocateElement(Convert.ToInt32(splStr[a]), typeof(MM_Transformer), AddIfNew) as MM_Transformer);
            ...
            else if (OutgoingType == typeof(List<MM_KVLevel>))
            {
                ...
                List<MM_KVLevel> OutLevels = new List<MM_KVLevel>(splStr.Length);
                for (int a = 0; a < splStr.Length; a++)
                    OutLevels.Add(MM_Repository.KVLevels[splStr[a]]);
                AssignValue(fpI, OutLevels, this);
            }
            ...
            else if (OutgoingType == typeof(List<MM_Load>))
            {
                String[] splStr = OutgoingValue.Split(',');
                List<MM_Load> OutLoads = new List<MM_Load>(splStr.Length);
                for (int a = 0; a < splStr.Length; a++)
                    OutLoads.Add(Data_Integration.LocateElement(Convert.ToInt32(splStr[a]), typeof (MM_Load), AddIfNew) as MM_Load);
                AssignValue(fpI, OutLoads, this);
            }
            else if (OutgoingType == typeof(List<MM_Unit>))
            {
                String[] splStr = OutgoingValue.Split(',');
                List<MM_Unit> OutUnits = new List<MM_Unit>(splStr.Length);
                for (int a = 0; a < splStr.Length; a++)
                    OutUnits.Add(Data_Integration.LocateElement(Convert.ToInt32(splStr[a]), typeof (MM_Unit), AddIfNew) as MM_Unit);
                AssignValue(fpI, OutUnits, this);
            }
            else if (OutgoingType == typeof(List<MM_Breaker_Switch>))
            {
                String[] splStr = OutgoingValue.Split(',');
                List<MM_Breaker_Switch> OutUnits = new List<MM_Breaker_Switch>(splStr.Length);
                for (int a = 0; a < splStr.Length; a++)
                    OutUnits.Add(Data_Integration.LocateElement(Convert.ToInt32(splStr[a]), typeof (MM_Breaker_Switch), AddIfNew) as MM_Breaker_Switch);
                AssignValue(fpI, OutUnits, this);
            }
            else if (OutgoingType == typeof(List<int>))
            {
                String[] splStr = OutgoingValue.Split(',');
                List<int> ints = new List<int>(splStr.Length);
                for (int a = 0; a < splStr.Length; a++)
                {
                    int o = 0;
                    Int32.TryParse(splStr[a].Trim(), out o);
                    ints.Add(o);
                }
                AssignValue(fpI, ints, this);
            }
            ...
            else if (OutgoingType == typeof(MM_Unit_Logical[]))
            {
                String[] splStr = OutgoingValue.Split(',');
                MM_Unit[] OutUnits = new MM_Unit[splStr.Length];
                for (int a = 0; a < splStr.Length; a++)
                    OutUnits[a] = Data_Integration.LocateElement(Convert.ToInt32(splStr[a]), typeof (MM_Unit_Logical), AddIfNew) as MM_Unit;
                AssignValue(fpI, OutUnits, this);
            }
            ...
            else if (OutgoingType == typeof(List<MM_Electrical_Bus>))
            {
                String[] splStr = OutgoingValue.Split(',');
                List<MM_Electrical_Bus> OutElectricalBus = new List<MM_Electrical_Bus>();
                for (int a = 0;a < splStr.Length;a++)
                    OutElectricalBus.Add(Data_Integration.LocateElement(Convert.ToInt32(splStr[a]), typeof(MM_Electrical_Bus), AddIfNew) as MM_Electrical_Bus);
                AssignValue(fpI, OutElectricalBus, this);
            }
            else if (OutgoingType == typeof(List<MM_ShuntCompensator>))
            {
                String[] splStr = OutgoingValue.Split(',');
                List<MM_ShuntCompensator> OutElectricalBus = new List<MM_ShuntCompensator>();
                for (int a = 0;a < splStr.Length;a++)
                    OutElectricalBus.Add(Data_Integration.LocateElement(Convert.ToInt32(splStr[a]), typeof(MM_ShuntCompensator), AddIfNew) as MM_ShuntCompensator);
                AssignValue(fpI, OutElectricalBus, this);
            }
            else if (OutgoingType == typeof(List<MM_Bus>))
            {
                String[] splStr = OutgoingValue.Split(',');
                List<MM_Bus> OutElectricalBus = new List<MM_Bus>();
                for (int a = 0;a < splStr.Length;a++)
                    OutElectricalBus.Add(Data_Integration.LocateElement(Convert.ToInt32(splStr[a]), typeof(MM_Bus), AddIfNew) as MM_Bus);
                AssignValue(fpI, OutElectricalBus, this);
            }
            else if (OutgoingType == typeof(List<MM_Node>))
            {
                String[] splStr = OutgoingValue.Split(',');
                List<MM_Node> OutElectricalBus = new List<MM_Node>();
                for (int a = 0;a < splStr.Length;a++)
                    OutElectricalBus.Add(Data_Integration.LocateElement(Convert.ToInt32(splStr[a]), typeof(MM_Node), AddIfNew) as MM_Node);
                AssignValue(fpI, OutElectricalBus, this);
            }
            else if (OutgoingType == typeof (MM_Contingency))
            {
                int result = 0;
                if (Int32.TryParse(OutgoingValue, out result))
                    AssignValue(fpI, Data_Integration.LocateElement(result, typeof (MM_Contingency), AddIfNew), this);
                else if (OutgoingValue != null)
                {
                    MM_Contingency con = null;
                    MM_Repository.Contingencies.TryGetValue(OutgoingValue, out con);
                    AssignValue(fpI, con, this);
                }
            ...
            else if (OutgoingType == typeof (List<MM_Element>))
            {
                String[] splStr = OutgoingValue.Split(',');
                List<MM_Element> OutElems = new List<MM_Element>(splStr.Length);
                for (int a = 0; a < splStr.Length; a++)
                    if (!string.IsNullOrEmpty(splStr[a]))
                        OutElems.Add(Data_Integration.LocateElement(Convert.ToInt32(splStr[a]), typeof (MM_Element), AddIfNew) as MM_Element);
                AssignValue(fpI, OutElems, this);
            }
            else if (OutgoingType == typeof(System.Windows.Forms.ListViewItem)) {}
            else
            {
                MM_System_Interfaces.WriteConsoleLine("Unable to handle type " + fpI.ReferenceType.ToString() + " for " + fpI.Name);
            }
        ...
        public static void WriteXMLData(String Name, Object Value, XmlTextWriter xW)
        {
            ...
            else if (Value is MM_Unit_Type)
                xW.WriteAttributeString(Name, (Value as MM_Unit_Type).Name);
