...
namespace MacomberMapClient.User_Interfaces.Violations
{
    /// <summary>
    /// © 2014, Michael E. Legatt, Ph.D., Electric Reliability Council of Texas, Inc. All Rights Reserved.
    /// This control displays violations, as customizable by the user
    /// </summary>
    public partial class MM_Violation_Viewer : UserControl
    {
        ...
        private MM_OneLine_Viewer oneLine;
        ...
        private Dictionary<MM_AlarmViolation, MM_AlarmViolation> RetrieveUIDs()
        {
            ...
            else
            {
                ...
                foreach (MM_AlarmViolation Viol in ShownViolations.Keys.ToArray())
                    if (ShownViolations.TryGetValue(Viol, out FoundItem) && FoundItem.Index < lvViolations.Items.Count)                      
        ...
        public MM_Violation_Viewer()
        {
            ...
            lvViolations.GetType().GetProperty("DoubleBuffered", System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic).SetValue(lvViolations, true);            
        ...
        public void SetControls( MM_Network_Map_DX networkMap, MM_Element[] BaseElements)
        {
            this.BaseElements = BaseElements;
            SetControls( networkMap);
        }
        ...
        public void SetControls(MM_OneLine_Viewer oneLine)
        {
            this.oneLine = oneLine;
        }

        /// <summary>
        /// Update our base element
        /// </summary>
        /// <param name="BaseElements"></param>
        public void UpdateBaseElement(MM_Element[] BaseElements)
        {
            this.BaseElements = BaseElements;

            //Set up our columns
            lvViolations.Columns.Clear();
        ...
        /// <param name="miniMap">The associated mini-map</param>
        /// <param name="networkMap">The associated network map</param>
        public void SetControls( MM_Network_Map_DX networkMap)
        {
        ...
        private void Repository_ViewChanged(MM_Display_View ActiveView)
        {
            ...
            else
            {
                ...
                try
                {
                    if (ShownViolations != null && ShownViolations.Values.Count > 0)
                    {
                        foreach (var l in ShownViolations.Values)
            ...
            if (this.BaseElements != null)
            {
                ...
                foreach (MM_Element BaseElement in BaseElements)
                    if (BaseElement is MM_Contingency)
                        ...
                        Include = false;
                    else if (Violation.ViolatedElement is MM_Bus && Violation.ViolatedElement.Substation == null)
                        Include = false;
                    else
                        Include = true;
                if (!Include)
        ...
        {
            if ((Violation.ViolatedElement is MM_Bus && Violation.ViolatedElement.Substation == null) || Violation.ViolatedElement.Name == null)
                return;
            ...
            NewLVItem.SubItems.Add(Violation.ViolatedElement.ToString());
            ...
            {
                if (Violation.KVLevel == null)
                    NewLVItem.ToolTipText = Violation.Type.Name + " - " + Violation.ViolatedElement.ElemType.Name + " at " + Violation.EventTime.ToString(MILITARY_TIME_FORMAT);
                else
                    NewLVItem.ToolTipText = Violation.Type.Name + " - " + Violation.ViolatedElement.ElemType.Name + " - " + Violation.KVLevel.Name + " at " + Violation.EventTime.ToString(MILITARY_TIME_FORMAT);
            } catch (Exception ex)
            {
                Console.WriteLine(ex.ToString());
            }
            //Check each item's width
            for (int x = 0;x < NewLVItem.SubItems.Count && x < lvViolations.Columns.Count;x++)
            {
            ...
            if (ViolationsChanged != null)
                ViolationsChanged.Invoke(this, EventArgs.Empty);
        }
        ...
        private void CreateSummaryButtons()
        {
            ...
            foreach (MM_AlarmViolation_Type ViolType in MM_Repository.ViolationTypes.Values)
            {
                ...
                foreach (MM_KVLevel KVLevel in ViolType.ViolationsByVoltage.Keys)
                    if (!TypeVoltageButtons.ContainsKey(new KeyValuePair<MM_AlarmViolation_Type, MM_KVLevel>(ViolType, KVLevel)))
                ...
                foreach (MM_Element_Type ElemType in ViolType.ViolationsByElementType.Keys)
                    if (!TypeElementButtons.ContainsKey(new KeyValuePair<MM_AlarmViolation_Type, MM_Element_Type>(ViolType, ElemType)))
        ...
        private AlarmViolation_Button CreateToolStripButton(int ImageNumber, String ButtonTitle, Color TextColor, bool Visible, MM_AlarmViolation_Type BaseType, Object SecondaryValue)
        {
            ...
            SummaryItem.Checked = BaseType.ViolationViewerDefault;
            ...
            else if (SummaryItem.Owner != null)
                SummaryItem.BackColor = SummaryItem.Owner.BackColor;
            else
                SummaryItem.BackColor = Color.White;
        ...
        private void ViolationViewer_Resize(object sender, EventArgs e)
        {
            try
            {
                lvViolations.TileSize = new Size(DisplayRectangle.Width - 24, lvViolations.Font.Height * (lvViolations.Columns.Count+1));
            }
            catch (Exception)
            { }
        }
        ...
        private void PopupMenu(ListViewHitTestInfo ht, Point Location)
        {
            ...
            if (ht.Item.Selected && ht.Item.ListView.SelectedIndices.Count > 1)
            {
                ...
                ListView.SelectedIndexCollection col = ht.Item.ListView.SelectedIndices;

                foreach (int item in col)
                {
                    var I = ht.Item.ListView.Items[item];
        ...
        public class ListViewItemComparer : IComparer<ListViewItem>, IComparer
        {
            ...
            public ComparisonTypeEnum ComparisonType = ComparisonTypeEnum.ViolType;
        ...
        partial class AlarmViolation_Renderer : ToolStripProfessionalRenderer
        {
            ...
            private MM_Violation_Viewer Parent;

            /// <summary>
            /// Initialize the renderer
            /// </summary>
            /// <param name="Parent"></param>
            public AlarmViolation_Renderer(MM_Violation_Viewer Parent)
            {
                this.Parent = Parent;
            }
            ...
            protected override void OnRenderButtonBackground(ToolStripItemRenderEventArgs e)
            {
                ...
                ControlPaint.DrawBorder(e.Graphics, DrawRect, Color.Lime, (e.Item as ToolStripButton).Checked ? ButtonBorderStyle.Solid : ButtonBorderStyle.None);
        ...
        public void SetSort(MM_Violation_Viewer.ListViewItemComparer.ComparisonTypeEnum SortType)
        {
        ...
        public void SetSplit(MM_Violation_Viewer.ListViewItemComparer.ComparisonTypeEnum SplitType)
        {
        ...
        public void SetGrouping(MM_Violation_Viewer.ListViewItemComparer.ComparisonTypeEnum GroupType)
        {
        ...
        public event EventHandler ViolationsChanged;
    }
}