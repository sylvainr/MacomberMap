...
#region MacomberMap.UI.Win32Imports

namespace MacomberMap.UI.Private.Win32Imports
{
    ...
    public class Win32
    {
        ...
        public class WM
        {
            public const int Wi_MOUSEMOVE = 0x0200;
            public const int Wi_NCMOUSEMOVE = 0x00A0;
            public const int Wi_NCLBUTTONDOWN = 0x00A1;
            public const int Wi_NCLBUTTONUP = 0x00A2;
            public const int Wi_NCLBUTTONDBLCLK = 0x00A3;
            public const int Wi_LBUTTONDOWN = 0x0201;
            public const int Wi_LBUTTONUP = 0x0202;
            public const int Wi_KEYDOWN = 0x0100;
        }
...
{
    using MacomberMap.UI.Private.Win32Imports;
    ...
    public class StickyWindow : System.Windows.Forms.NativeWindow
    {
        ...
        private bool ResizeMsgProcessor(ref Message m)
        {
            ...
            switch (m.Msg)
            {
                case Win32.WM.Wi_LBUTTONUP:
                    {	// ok, resize finished !!!
                        EndResize();
                        break;
                    }
                case Win32.WM.Wi_MOUSEMOVE:
                    {
                        mousePoint.X = (short)Win32.Bit.LoWord((int)m.LParam);
                        mousePoint.Y = (short)Win32.Bit.HiWord((int)m.LParam);
                        Resize(mousePoint);
